...
# Img = Image
#directories, constants, and filetypes
acptImgFt = (".jpeg", ".png", ".tif")
shiftedDir = 'C:/Users/aadvi/Desktop/Coding/T62/Frame Images/Cyl images/Cyl reformatted/'
reformattedDir = 'C:/Users/aadvi/Desktop/Coding/T62/Frame Images/Cyl images/Cyl reformatted/'
csvDir = 'C:/Users/aadvi/Desktop/Coding/T62/Frame Images/Lat data/t62_cyl_coords.csv'
i = 0
iminfo = [shiftedDir, "this will be replaced in two ms", i ]
#crop order: bottom, top, left, right
crop = [60, 80, 0, 100]
...
def shift (csv, image,lat, lon, crop):
    ...
    return nsa_lats, nsa_lons, hc_band
def ImageToAnalyze (image, csv, directoryList, cropParams):
    lat = Jcube_to_csv(geo_csv = csv, var='lat', nL=125, nS=133)
    lon = Jcube_to_csv(geo_csv = csv, var='lon', nL=125, nS=133)
    vals = []
    x, y = [1,2,3,4], [1,2,3,4]
    #print(image)
    nsa_lats, nsa_lons, uncroppedimgdata  = shift(csv, image,lat, lon,cropParams)
    #crop image
    #get data from image
    #run lobf on line
    
list = os.listdir(shiftedDir) # dir is your directory path
number_files = len(os.listdir(shiftedDir))
print ("Number of files in directory: ",number_files)
for filename in os.listdir(shiftedDir):

    if filename.endswith(acptImgFt): 
        iminfo[1] = filename
        imageFN = str(iminfo[0] + iminfo[1])
        ImageToAnalyze(imageFN, csvDir, iminfo,crop)
        i+=1
        iminfo[2] = i