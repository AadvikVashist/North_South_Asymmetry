...
def imageCanOpen(image):
    ...
    try:
        ...
        print("image " + image + " cannot be opened")
        return False
    print( 'image "' + image + '" was loaded succesfully.' )
    return True
#Brightness of Image
...
#Main Code   
def ImagetoAnalyze(image, loopnum):
    loopnumi = loopnum
    yes = (imageCanOpen(image))
    img = Image.open(image)
    ITA = img.load()
    ...
    for ximg in range (0,width,1):
        ...
        sextRSQavg += sextRSQ
        colNum += 1
        pixelRGBmaxindex = pixelRGB.index(max(pixelRGB))
        pixelRGBmax.append(pixelRGB[pixelRGBmaxindex])
        LatXmax.append(LatX[pixelRGBmaxindex])
        pixelRGB.clear()
    str = [loopnumi]
    wks.update_col((loopnum-2),str, row_offset=0)
    wks.update_col((loopnum-2),LatXmax, row_offset=1)
    wks.update_col((loopnum-1),pixelRGBmax, row_offset=1)
    time.sleep(5)
    plt.plot(LatXmax, pixelRGBmax, 'o')
    plt.xlim([-90, 90])
    plt.ylim([0,255])
    print(colNum, " \n" , linearRSQavg / colNum, " \n",quadRSQavg / colNum," \n",cubRSQavg / colNum , " \n",quartRSQavg / colNum, " \n", quintRSQavg / colNum, " \n", sextRSQavg / colNum)
...
#-90,90 for the x values in model . Max Y value = data need to be collected. store data in lists. 
#splice image to y = 60-80, x = 20 values towards end that have dead pixels
#get rid of artifcats (remove mode of value pixels)
#Get datapoints
#Plot linregress using datapoints. Return R value
