...
def shift (csv, image, lat, lon, crop):
    ...
    for col in range(imW): # 133 = image width
        ...
        imCol = im[:,col, 0]
        shift_IF_down  = np.insert(imCol, [0], [np.nan,np.nan])
        shift_IF_up  = np.concatenate((imCol,[np.nan],[np.nan]))
        ...
        shift_IF_up = np.concatenate((imCol, nans))
        #shift_IF_down  = np.insert(im[:,col], [0,0], [np.nan,np.nan])
        #shift_IF_up  = np.concatenate((im[:,col],[np.nan],[np.nan]))
        hc_band[:,col] = (shift_IF_up - shift_IF_down)[num_of_nans:-(num_of_nans)]
        ## subset HC band column b/t 25°S to 0°N 
        hc_band[crop[0]:crop[1],crop[2]:crop[3]]
        if_sh = hc_band[:,col][(lat[:,col] > -25.0) & (lat[:,col] < 0.0)]
        lat_sh = lat[:,col][(lat[:,col] > -25.0) & (lat[:,col] < 0.0)]
        lon_sh = lon[:,col][(lat[:,col] > -25.0) & (lat[:,col] < 0.0)]
