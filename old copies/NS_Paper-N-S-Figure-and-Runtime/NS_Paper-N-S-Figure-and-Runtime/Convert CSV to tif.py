...
import pandas as pd
class tif:
    def __init__(self, file, nS, nL, namingScheme):
        #set all self directory variables
        self.file = file
        self.nL = nL
        self.nS = nS
        for bander in range(96):
            band = self.Jcube_to_csv(csv = file, nL = self.nL, nS = self.nS, bands = bander)
            im = Image.fromarray(np.uint8(band * (255/np.max(band))) , 'L')
            im.save((namingScheme + str(bander) + '.tif'))
...
#t92 = tif('C:/Users/aadvi/Desktop/Titan Paper/Data/T92/t92_nsa_cyl.csv', 725, 361, 'C:/Users/aadvi/Desktop/Titan Paper/Data/T92/vis.cyl/T92.')